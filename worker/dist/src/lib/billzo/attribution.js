...
exports.attributeRecovery = attributeRecovery;
exports.getInvoiceRecoveryTimeline = getInvoiceRecoveryTimeline;
exports.getTenantRecoveryMetrics = getTenantRecoveryMetrics;
 ...
 */
async function attributeRecovery(params) {
    const { invoiceId, tenantId, paymentId, paymentTimestamp, attributionWindowHours = 48, } = params;
    ...
    const { data: reminderEvents, error } = await supabase_admin_1.supabaseAdmin
        .from('outbox')
        .select('*')
    ...
    const { data: attribution, error: attributionError } = await supabase_admin_1.supabaseAdmin
    ...
    await (0, events_1.emitRecoveryCompleted)({
        invoiceId,
        tenantId,
 ...
 */
async function getInvoiceRecoveryTimeline(invoiceId) {
    // Get all outbox events for this invoice
    const { data: events, error } = await supabase_admin_1.supabaseAdmin
        .from('outbox')
        .select('*')
        .eq('entity_id', invoiceId)
        .in('type', [
    ...
    const { data: attributions } = await supabase_admin_1.supabaseAdmin
        .from('recovery_attributions')
        .select('*')
 ...
 */
async function getTenantRecoveryMetrics(tenantId) {
    // Get total outstanding
    const { data: invoices } = await supabase_admin_1.supabaseAdmin
        .from('invoices')
    ...
    const { data: paidInvoices } = await supabase_admin_1.supabaseAdmin
        .from('invoices')
