...
exports.buildAutomationPlan = buildAutomationPlan;
exports.formatPlanTime = formatPlanTime;
function buildAutomationPlan(cases, upcoming) {
    const plan = [];
    const now = new Date();
    // 1. Scheduled reminders from upcoming
    for (const r of upcoming) {
        if (!r.nextRecoveryAt || r.isPending)
            continue;
    ...
    plan.sort((a, b) => {
        if (a.status === 'manual_required' && b.status !== 'manual_required')
            return -1;
        if (a.status !== 'manual_required' && b.status === 'manual_required')
            return 1;
        ...
        if (a.nextAction.scheduledAt)
            return -1;
        if (b.nextAction.scheduledAt)
            return 1;
        return 0;
    });
    return plan;
...
//# sourceMappingURL=buildAutomationPlan.js.map