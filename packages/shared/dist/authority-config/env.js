...
function requireEnv(env, key) {
    const val = env[key];
    if (!val) {
        throw new Error(`AuthorityConfig: missing required env variable "${key}". ` +
            `Set it in .env or process environment before parseEnv() is called.`);
    }
    return val;
}
function parseHmacSecrets(env) {
...
function parseEnv(env) {
    ...
    if (raw.transportTimeoutMs < 100)
        throw new Error('AuthorityConfig: transportTimeoutMs must be >= 100');
    if (raw.leaseTtlMs < 1000)
        throw new Error('AuthorityConfig: leaseTtlMs must be >= 1000');
