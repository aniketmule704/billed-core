    ...
    <div className="min-h-screen bg-slate-50 pb-8">
      <div className="max-w-5xl mx-auto px-4 lg:px-8 py-5 lg:py-8 space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-slate-900">Recovery History</h1>
        ...
        {tab === "reminders" && (
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-slate-200"
            >
          ...
          <>
            {tab === "reminders" && events.length === 0 && (
              <div className="bg-white border border-dashed border-slate-200 rounded-lg p-12 text-center">
                ...
                <p className="font-semibold text-slate-700">{search ? "No reminders match your search" : "No reminders sent yet"}</p>
                <p className="text-xs text-slate-500 mt-1">{search ? "Try a different search term." : "Reminders will appear here once sent."}</p>
              </div>
            )}
            {tab === "timeline" && timeline.length === 0 && (
              <div className="bg-white border border-dashed border-slate-200 rounded-lg p-12 text-center">
                <Clock className="h-8 w-8 text-slate-300 mx-auto mb-3" />
                <p className="font-semibold text-slate-700">No recovery activity yet</p>
                <p className="text-xs text-slate-500 mt-1">Promises, payments, calls, and reminders will appear here.</p>
              </div>
            )}
          </>
        )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50">
                    ...
                    <th className="text-right px-4 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Amount</th>
                    <th className="text-center px-4 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Stage</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {events.map((evt) => {
                    ...
                    const stageCls = STAGE_COLORS[evt.stage] || "bg-slate-100 text-slate-600"
                    const statusCls = STATUS_BADGES[evt.status] || "bg-slate-100 text-slate-600"
                    return (
                      <tr key={evt.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-4 py-3 text-xs text-slate-600 whitespace-nowrap">
                          <p>{formatDate(evt.occurredAt)}</p>
                          <p className="text-[10px] text-slate-400">{formatTime(evt.occurredAt)}</p>
                        </td>
                        <td className="px-4 py-3">
                          <p className="text-sm font-medium text-slate-900">{evt.customerName}</p>
                          {evt.customerPhone && (
                            <p className="text-[11px] text-slate-500 font-mono">{evt.customerPhone}</p>
                          )}
                        </td>
                        <td className="px-4 py-3 text-right text-sm font-semibold text-slate-900 tabular-nums whitespace-nowrap">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={page <= 1}
                  className="flex items-center gap-1 px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-medium text-slate-600 bg-white hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed"
                >
        ...
        {!loading && tab === "timeline" && timeline.length > 0 && (
          <div className="space-y-3">
            {timeline.map((evt) => {
              ...
              const typeColor = TYPE_COLORS[evt.type] || "bg-slate-100 text-slate-600"
              return (
                <div key={evt.id} className="bg-white border border-slate-200 rounded-xl p-4 hover:shadow-sm transition-shadow">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="font-semibold text-slate-900">{evt.customerName}</p>
                      ...
                      <div className="flex items-center gap-3 mt-1.5 text-[10px] text-slate-400">
                        ...
                        {evt.amount > 0 && <span className="font-medium text-slate-900">{formatINR(evt.amount)}</span>}
