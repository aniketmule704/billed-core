    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border bg-white p-5 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-widest text-muted-foreground">AI Daily Brief</p>
            <h2 className="mt-1 text-lg font-black">
              {brief?.headline || 'Preparing today\'s business priorities...'}
            </h2>
            <p className="mt-1 text-xs text-muted-foreground">
              {brief?.source === 'gemini' ? 'Generated by Gemini Flash' : 'Smart fallback brief'}
            </p>
          </div>
          <button
            onClick={() => loadDailyBrief(true)}
            disabled={briefLoading}
            className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-input hover:bg-secondary disabled:opacity-50"
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {(brief?.actions || []).map((action) => (
            <button
              ...
              className={`rounded-xl border p-4 text-left transition-colors hover:bg-secondary/60 ${
            ...
            >
              <span className={`text-[10px] font-black uppercase tracking-widest ${
                action.priority === 'high' ? 'text-red-600' : action.priority === 'medium' ? 'text-yellow-700' : 'text-muted-foreground'
              }`}>
                {action.priority}
              </span>
              <h3 className="mt-1 font-black">{action.title}</h3>
              <p className="mt-1 text-xs font-medium text-muted-foreground">{action.detail}</p>
              <span className="mt-3 inline-block rounded-lg bg-foreground px-3 py-2 text-xs font-black text-white">
                {action.actionLabel}
              </span>
            </button>
          ))}
          ...
          {!brief && (
            <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground md:col-span-3">
              {briefLoading ? 'Gemini is reading today\'s sales, credit and stock signals...' : 'Brief will appear here after your data loads.'}
            </div>
          )}
        </div>
      </section>
